...
struct trdispatch_mcu {
    ...
    uint32_t trsync_oid, set_timeout_msgid, trigger_msgid;
...
{
    uint32_t msg[3] = {
        tdm->trigger_msgid, tdm->trsync_oid, tdm->td->dispatch_reason
    };
...
{
    uint32_t msg[3] = {
        tdm->set_timeout_msgid, tdm->trsync_oid, tdm->expire_clock
    };
    struct queue_message *qm = message_alloc_and_encode(msg, ARRAY_SIZE(msg));
...
trdispatch_mcu_alloc(struct trdispatch *td, struct serialqueue *sq
                     ...
                     , uint32_t set_timeout_msgid, uint32_t trigger_msgid
                     , uint32_t state_msgid)
{
    ...
    tdm->set_timeout_msgid = set_timeout_msgid;
    tdm->trigger_msgid = trigger_msgid;

    // Setup fastreader to match trsync_state messages
    uint32_t state_prefix[] = {state_msgid, trsync_oid};
