...
class MCU_trsync:
    ...
    def _build_config(self):
        ...
        set_timeout_id = mcu.lookup_command_id(
            "trsync_set_timeout oid=%c clock=%u")
        trigger_id = mcu.lookup_command_id("trsync_trigger oid=%c reason=%c")
        state_id = mcu.lookup_command_id(
            "trsync_state oid=%c can_trigger=%c trigger_reason=%c clock=%u")
        ffi_main, ffi_lib = chelper.get_ffi()
        self._trdispatch_mcu = ffi_main.gc(ffi_lib.trdispatch_mcu_alloc(
            self._trdispatch, mcu._serial.serialqueue, # XXX
            self._cmd_queue, self._oid, set_timeout_id, trigger_id,
            state_id), ffi_lib.free)
    def _shutdown(self):
